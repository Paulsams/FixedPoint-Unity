...
namespace Paulsams.FixedPoint
{
    public partial struct fixmath
    {
        ...
        public static fp2 Clamp(fp2 num, fp2 min, fp2 max)
        {
            ...
            r.x = num.x.value < min.x.value
                ? min.x
                : num.x.value > max.x.value
                    ? max.x : num.x;

            r.y = num.y.value < min.y.value
                ? min.y
                : num.y.value > max.y.value
                    ? max.y : num.y;
